...
The above functions can be used as a starting point for building a custom shot chart. However, the package also includes a function `shotChart` that builds a basic complete shot chart. This function returns a list with two elements: the plot and the raw data from which the plot was built. With no arguments, this function will produce a default chart for Steph Curry.
